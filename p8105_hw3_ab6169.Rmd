...
The table shows roughly similar counts of men and women within each education category. There are 12 more women than men in the "High School Equivalent" category. There are slightly more participants in the “More than High School” group.
...
The age distributions show that participants with higher education levels tend to be older on average.
Within each education category, the age patterns for men and women are similar, but women appear slightly more represented among older age groups in the “More than High School” panel.


Introducing Total Activity Variable
```{r message=FALSE}

##Aggregated Data With Total Activity Variable
total_activity_df =
  nhanes_merged |> 
...
plot_total_activity =
  ...
  labs(
    ...
    subtitle = "Each point represents one participant.Smooth lines show trends by sex",
    x = "Age (years)",
...
```
Comments: 
The plot shows how total daily activity varies with age and education level for men and women. Among participants with less than a high school education, males generally have slightly higher total activity than females across most ages. But, both groups show a gradual decline in activity with age. For those with a high school equivalent or more than high-school education, females showcase higher total activity than males. The female lines remain consistently above the male lines in these two panels. This suggests to me that there is greater daily movement among more educated women.


3 panel plot
```{r}
activity_timecourse_df =
  nhanes_merged |> 
  ...
  geom_smooth(se = FALSE, linewidth = 1.3, linetype = "dashed") +
  facet_wrap(~ education) +
  labs(
    title = "24-Hour Activity Time Course by Education Level",
    subtitle = "Average MIMS values across minutes of the day, separated by sex and education",
...
This plot shows daily patterns across all education levels. Activity levels are lowest during nighttime hours, such as 0–400 and 1200–1440. I see that activity peaks in the morning and also the afternoon. Across all three panels, I can see that men and females follow similar daily patterns, but there are some minor differences. In the less than high school group, I observed that men show slightly higher early morning peaks. Meanwhile, in the high school equivalent and more than high school groups, women display higher afternoon activity levels. 
